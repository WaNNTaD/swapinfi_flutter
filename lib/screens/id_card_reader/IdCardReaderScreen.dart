...
class IdCardReaderScreen extends StatefulWidget {
  ...
  IdCardReaderScreen({required this.secureStorageService});
  @override
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line.contains('nom') &&
        ...
        if (!ignoreList.contains(nextLine) &&
          ...
          print('Prénom: ${extractedData['Prénom']}');
          print('Autres prénoms: ${extractedData['Autres prénoms']}');
          fieldsFound["Prénom"] = true;
        }
      } else if (line.contains('sexe') && !fieldsFound["Sexe"]!) {
        String nextLine = lines[i + 1].trim();
        if (nextLine.toLowerCase().contains('sex') &&
            i + 2 < lines.length &&
            RegExp(r'^[a-zA-Z]/[a-zA-Z]$').hasMatch(lines[i + 2].trim())) {
          extractedData['Sexe'] = lines[i + 2].trim();
          print('Sexe: ${lines[i + 2].trim()}');
